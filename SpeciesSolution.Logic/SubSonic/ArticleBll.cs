...
namespace SpeciesSolution.Logic.SpeciesQuery {
	...
	/// Article表逻辑类
	/// </summary>
	public partial class ArticleBll : LogicBase {
		...
		private const string const_CacheKey = "Cache_Article";
        private const string const_CacheKey_Date = "Cache_Article_Date";

		#region 单例模式
		//定义单例实体
		private static ArticleBll _ArticleBll = null;

		/// <summary>
		/// 获取本逻辑类单例
		/// </summary>
		/// <returns></returns>
		public static ArticleBll GetInstence() {
			if (_ArticleBll == null) {
        ...
        public DataAccess.Model.Article GetModel(long id, bool isCache = true)
        {
            //判断是否使用缓存
		    if (CommonBll.IsUseCache() && isCache)
		    {
		    ...
		    {
                //从数据库中直接读取
                var model = Article.SingleOrDefault(x => x.Id == id);
                if (model == null)
                {
                    return null;
                }
        ...
        /// <returns>DataAccess.Model.Article</returns>
        public DataAccess.Model.Article GetModelForCache(string conditionColName, object value)
        {
			try
            {
                //从缓存中获取List
                var list = GetList();
                DataAccess.Model.Article model = null;
                ...
                switch (conditionColName)
                {
					...
					case "ArticleName" :
						model = list.SingleOrDefault(x => x.ArticleName == (string)value);
                        expression = x => x.ArticleName == (string)value;
                        break;
                ...
                {
                    //从数据库中读取
                    var tem = Article.SingleOrDefault(expression);
                    if (tem == null)
                    {
                        return null;
                    }
        ...
        /// <returns>DataAccess.Model.Article</returns>
        public DataAccess.Model.Article GetModelForCache(Expression<Func<DataAccess.Model.Article, bool>> expression)
        {
			//从缓存中读取记录列表
			var list = GetList();
            //如果条件为空，则查询全表所有记录
        ...
        public void SetModelForCache(DataAccess.Model.Article model)
        {
		    ...
		    if (list == null)
		    {
                list = new List<DataAccess.Model.Article>();
		    }
            //添加记录
            list.Add(model);
        }
        ...
        /// </summary>
        /// <param name="model">记录实体</param>
        public void SetModelForCache(Article model)
        {
		...
		/// <summary>
        /// 按条件删除IIS缓存中Article表的指定记录
        /// </summary>
        /// <param name="expression">条件，值为null时删除全有记录</param>
		public void DelCache(Expression<Func<DataAccess.Model.Article, bool>> expression)
        {
            //从缓存中获取List
		    var list = GetList();
            //如果缓存为null，则不做任何处理
		...
		#region 实体转换
		/// <summary>
		/// 将Article记录实体（SubSonic实体）转换为普通的实体（DataAccess.Model.Article）
		/// </summary>
        /// <param name="model">SubSonic插件生成的实体</param>
		/// <returns>DataAccess.Model.Article</returns>
		public DataAccess.Model.Article Transform(Article model)
        {			
			if (model == null) 
				return null;

            return new DataAccess.Model.Article
            {
		...
		/// <summary>
		/// 将Article记录实体集（SubSonic实体）转换为普通的实体集（DataAccess.Model.Article）
		/// </summary>
        /// <param name="sourceList">SubSonic插件生成的实体集</param>
        public IList<DataAccess.Model.Article> Transform(IList<Article> sourceList)
        {
			//创建List容器
            var list = new List<DataAccess.Model.Article>();
		...
		/// <summary>
		/// 将Article记录实体由普通的实体（DataAccess.Model.Article）转换为SubSonic插件生成的实体
		/// </summary>
        /// <param name="model">普通的实体（DataAccess.Model.Article）</param>
		/// <returns>Article</returns>
		public Article Transform(DataAccess.Model.Article model)
        {
			if (model == null) 
				return null;
		...
		/// <summary>
		/// 将Article记录实体由普通实体集（DataAccess.Model.Article）转换为SubSonic插件生成的实体集
		/// </summary>
        /// <param name="sourceList">普通实体集（DataAccess.Model.Article）</param>
        public IList<Article> Transform(IList<DataAccess.Model.Article> sourceList)
        {
			//创建List容器
		...
		public void SetModelValue(DataAccess.Model.Article model, Dictionary<string, object> dic)
		{
		...
		public void SetModelValue(DataAccess.Model.Article model, string colName, object value)
		{
            ...
            switch (colName)
            {
				...
				case "ArticleName" :
					model.ArticleName = (string)value;
                    break;
        ...
        public int GetRecordCount()
        {
			...
			else
			{
				...
				return select.GetRecordCount<Article>();
			}
        }

		/// <summary>
		/// 获取Article表记录总数——从数据库中查询
		/// </summary>
        /// <param name="wheres">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(List<ConditionHelper.SqlqueryCondition> wheres) {
			var select = new SelectHelper();
			return select.GetRecordCount<Article>(wheres);

		}

		/// <summary>
		/// 获取Article表指定条件的记录总数——从数据库中查询
		/// </summary>
        /// <param name="expression">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(Expression<Func<Article, bool>> expression) {
            return new Select().From<Article>().Where(expression).GetRecordCount();
		}
        ...
        public IList<DataAccess.Model.Article> Find(Expression<Func<DataAccess.Model.Article, bool>> expression)
        {
			//从缓存中获取记录集
			var list = GetList();
            //判断获取记录集是否为null
        ...
        public bool Exist(int id)
        {
            ...
            //从数据库中查找
            return Article.Exists(x => x.Id == id);
        }
        ...
        /// <param name="expression">条件语句</param>
        /// <returns></returns>
        public bool Exist(Expression<Func<DataAccess.Model.Article, bool>> expression)
        {
            var list = GetList();
            if (list == null) 
                return false;
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionHelper.SqlqueryCondition> wheres = null, List<string> sorts = null) {
			try
            {
                //分页查询
                var select = new SelectHelper();
                return select.SelectDataTable<Article>(norepeat, top, columns, pageIndex, pageSize, wheres, sorts);
            }
            catch (Exception e)
            {
                //记录日志
                CommonBll.WriteLog("获取Article表记录时出现异常", e);
		...
		#region 添加与编辑Article表记录
		/// <summary>
		/// 添加与编辑Article记录
		/// </summary>
	    /// <param name="page">当前页面指针</param>
		/// <param name="model">Article表实体</param>
        /// <param name="content">更新说明</param>
        /// <param name="isCache">是否更新缓存</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
        public void Save(Page page, Article model, string content = null, bool isCache = true, bool isAddUseLog = true)
        {
			try {
				//保存
				model.Save();
				...
				if (isAddUseLog)
				{
					...
					{
						content = "{0}" + (model.Id == 0 ? "添加" : "编辑") + "Article记录成功，ID为【" + model.Id + "】";
					}
				...
				var result = "执行ArticleBll.Save()函数出错！";
		...
		/// <summary>
        /// 删除Article表记录——如果使用了缓存，删除成功后会清空本表的所有缓存记录，然后重新加载进缓存
        /// </summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="expression">条件语句</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
        public void Delete(Page page, Expression<Func<Article, bool>> expression, bool isAddUseLog = true)
        {
			//执行删除
			Article.Delete(expression);
			...
			if (isAddUseLog)
		    {
				...
				//UseLogBll.GetInstence().Save(page, "{0}删除了Article表记录！");
			}
        }
	    ...
	    public object GetFieldValue(string colName, string conditionColName, object value, bool isCache = true)
	    {
	        ...
	        else
	        {
				if (string.IsNullOrEmpty(conditionColName))
	            {
	                conditionColName = ArticleTable.Id;
	            }
        ...
        /// <param name="expression">条件</param>
        /// <returns></returns>
        public object GetFieldValue(string colName, Expression<Func<DataAccess.Model.Article, bool>> expression)
	    {
        ...
        {
            var select = new SelectHelper();
            return select.GetColumnsValue<Article>(colName, wheres);
        }
        ...
        /// <returns></returns>
		private object GetFieldValue(DataAccess.Model.Article model, string colName)
		{
		...
		#region 更新Article表指定字段值
		/// <summary>更新Article表记录指定字段值，如果使用了缓存，保存成功后会清空本表的所有缓存记录，然后重新加载进缓存</summary>
		/// <param name="page">当前页面指针</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionHelper.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			update.Update<Article>(dic, wheres);
			...
			if (isAddUseLog){
				...
				{
					//添加用户操作记录
					CommonBll.WriteLog("{0}修改了Article表记录。");
					//UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}修改了Article表记录。");				
				}
				else
				{
					//添加用户操作记录
		...
		#region 更新Article表指定主键Id的字段值
		/// <summary>更新Article表记录指定字段值</summary>
        /// <param name="page">当前页面指针</param>
        ...
        {
			content = content != "" ? content : "{0}修改了Article表主键Id值为" + id + "的记录。";
            ...
            if (id > 0)
            {
                ...
                wheres.Add(new ConditionHelper.SqlqueryCondition(ConstraintType.And, ArticleTable.Id, Comparison.Equals, id));
            };
        ...
        /// <summary>更新Article表记录指定字段值（更新一个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了Article表主键Id值为" + id + "的记录，将" + columnName + "字段值修改为" + columnValue;
            //设置更新字段
		 ...
		 /// <summary>更新Article表记录指定字段值（更新两个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了Article表主键Id值为" + id + "的记录，将" + columnName1 + "字段值修改为" + columnValue1 + "，" + columnName2 + "字段值修改为" + columnValue2;
            //设置更新字段
            var dic = new Dictionary<string, object>();
        ...
        /// <param name="isCache">是否从缓存中读取</param>
        /// <returns></returns>
        public string GetArticleName(Page page, int pkValue, bool isCache = true)
        {
            //判断是否启用缓存
            if (isCache && CommonBll.IsUseCache())
            {
                ...
                return model == null ? "" : model.ArticleName;
            }
            else
            {
                //从数据库中查询
                var model = Article.SingleOrDefault(x => x.Id == pkValue);
                return model == null ? "" : model.ArticleName;
            }
        }
        #endregion
