...
namespace SpeciesSolution.Logic.SpeciesQuery {
	...
	/// RefTable表逻辑类
	/// </summary>
	public partial class RefTableBll : LogicBase {
		...
		private const string const_CacheKey = "Cache_RefTable";
        private const string const_CacheKey_Date = "Cache_RefTable_Date";

		#region 单例模式
		//定义单例实体
		private static RefTableBll _RefTableBll = null;

		/// <summary>
		/// 获取本逻辑类单例
		/// </summary>
		/// <returns></returns>
		public static RefTableBll GetInstence() {
			if (_RefTableBll == null) {
				_RefTableBll = new RefTableBll();
			}
			return _RefTableBll;
		}
		#endregion
        ...
        /// <returns>DataAccess.Model.RefTable</returns>
        public DataAccess.Model.RefTable GetModelForCache(Expression<Func<DataAccess.Model.RefTable, bool>> expression)
        {
			//从缓存中读取记录列表
			var list = GetList();
            //如果条件为空，则查询全表所有记录
        ...
        public void SetModelForCache(DataAccess.Model.RefTable model)
        {
		    ...
		    if (list == null)
		    {
                list = new List<DataAccess.Model.RefTable>();
		    }
            //添加记录
            list.Add(model);
        }
		...
		/// <summary>
        /// 按条件删除IIS缓存中RefTable表的指定记录
        /// </summary>
        /// <param name="expression">条件，值为null时删除全有记录</param>
		public void DelCache(Expression<Func<DataAccess.Model.RefTable, bool>> expression)
        {
            //从缓存中获取List
		    var list = GetList();
            //如果缓存为null，则不做任何处理
		...
		/// <summary>
		/// 将RefTable记录实体集（SubSonic实体）转换为普通的实体集（DataAccess.Model.RefTable）
		/// </summary>
        /// <param name="sourceList">SubSonic插件生成的实体集</param>
        public IList<DataAccess.Model.RefTable> Transform(IList<RefTable> sourceList)
        {
			//创建List容器
            var list = new List<DataAccess.Model.RefTable>();
		...
		/// <summary>
		/// 将RefTable记录实体由普通的实体（DataAccess.Model.RefTable）转换为SubSonic插件生成的实体
		/// </summary>
        /// <param name="model">普通的实体（DataAccess.Model.RefTable）</param>
		/// <returns>RefTable</returns>
		public RefTable Transform(DataAccess.Model.RefTable model)
        {
			if (model == null) 
				return null;
		...
		/// <summary>
		/// 将RefTable记录实体由普通实体集（DataAccess.Model.RefTable）转换为SubSonic插件生成的实体集
		/// </summary>
        /// <param name="sourceList">普通实体集（DataAccess.Model.RefTable）</param>
        public IList<RefTable> Transform(IList<DataAccess.Model.RefTable> sourceList)
        {
			//创建List容器
            var list = new List<RefTable>();
		...
		public void SetModelValue(DataAccess.Model.RefTable model, Dictionary<string, object> dic)
		{
		...
		public void SetModelValue(DataAccess.Model.RefTable model, string colName, object value)
		{
            ...
            switch (colName)
            {
				...
				case "Taxa_Id" :
					model.Taxa_Id = (long)value;
                    break;

				case "Article_Id" :
					model.Article_Id = (long)value;
                    break;
        ...
        public int GetRecordCount()
        {
			...
			else
			{
				...
				return select.GetRecordCount<RefTable>();
			}
        }

		/// <summary>
		/// 获取RefTable表记录总数——从数据库中查询
		/// </summary>
        /// <param name="wheres">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(List<ConditionHelper.SqlqueryCondition> wheres) {
			var select = new SelectHelper();
			return select.GetRecordCount<RefTable>(wheres);

		}

		/// <summary>
		/// 获取RefTable表指定条件的记录总数——从数据库中查询
		/// </summary>
        /// <param name="expression">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(Expression<Func<RefTable, bool>> expression) {
            return new Select().From<RefTable>().Where(expression).GetRecordCount();
		}
        ...
        public IList<DataAccess.Model.RefTable> Find(Expression<Func<DataAccess.Model.RefTable, bool>> expression)
        {
			//从缓存中获取记录集
			var list = GetList();
            //判断获取记录集是否为null
        ...
        /// <param name="expression">条件语句</param>
        /// <returns></returns>
        public bool Exist(Expression<Func<DataAccess.Model.RefTable, bool>> expression)
        {
            var list = GetList();
            if (list == null) 
                return false;
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionHelper.SqlqueryCondition> wheres = null, List<string> sorts = null) {
			try
            {
                //分页查询
                var select = new SelectHelper();
                return select.SelectDataTable<RefTable>(norepeat, top, columns, pageIndex, pageSize, wheres, sorts);
            }
            catch (Exception e)
            {
                //记录日志
                CommonBll.WriteLog("获取RefTable表记录时出现异常", e);
		...
		{
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} = {2}", RefTableTable.TableName,  RefTableTable.Id, id);
		    ...
		    {
				//添加用户操作记录
				CommonBll.WriteLog("{0}删除了RefTable表id为【" + id + "】的记录！");
				//UseLogBll.GetInstence().Save(page, "{0}删除了RefTable表id为【" + id + "】的记录！");
			}
		}
		...
		public  void Delete(Page page, int[] id, bool isAddUseLog = true) 
		{
			...
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} in ({2})", RefTableTable.TableName,  RefTableTable.Id, str);
		    ...
		    {
				//添加用户操作记录
				CommonBll.WriteLog("{0}删除了RefTable表id为【" + str + "】的记录！");
				//UseLogBll.GetInstence().Save(page, "{0}删除了RefTable表id为【" + str + "】的记录！");
			}
		}
        ...
        {
            //设置Sql语句
            var sql = string.Format("delete from {0}", RefTableTable.TableName);
            ...
            if (isAddUseLog)
            {
                //添加用户操作记录
				CommonBll.WriteLog("{0}删除了RefTable表记录！");
                //UseLogBll.GetInstence().Save(page, "{0}删除了RefTable表所有记录！");
            }
        }
		#endregion
	    ...
	    {
		     //更新排序
			if (CommonBll.UpdateSort(page, grid1, tbxSort, "RefTable", sortName, "Article_Id"))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				CommonBll.WriteLog("{0}更新了RefTable表排序！");
				//UseLogBll.GetInstence().Save(page, "{0}更新了RefTable表排序！");
	    ...
	    {
		    //更新排序
			if (CommonBll.AutoSort("Article_Id", "RefTable", strWhere, isExistsMoreLv, pid, fieldName, fieldParentId))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				CommonBll.WriteLog("{0}对RefTable表进行了自动排序操作！");
				//UseLogBll.GetInstence().Save(page, "{0}对RefTable表进行了自动排序操作！");
	    ...
	    public object GetFieldValue(string colName, string conditionColName, object value, bool isCache = true)
	    {
	        ...
	        else
	        {
				if (string.IsNullOrEmpty(conditionColName))
	            {
	                conditionColName = RefTableTable.Id;
	            }
        ...
        /// <param name="expression">条件</param>
        /// <returns></returns>
        public object GetFieldValue(string colName, Expression<Func<DataAccess.Model.RefTable, bool>> expression)
	    {
        ...
        {
            var select = new SelectHelper();
            return select.GetColumnsValue<RefTable>(colName, wheres);
        }
        ...
        /// <returns></returns>
		private object GetFieldValue(DataAccess.Model.RefTable model, string colName)
		{
		...
		#region 更新RefTable表指定字段值
		/// <summary>更新RefTable表记录指定字段值，如果使用了缓存，保存成功后会清空本表的所有缓存记录，然后重新加载进缓存</summary>
		/// <param name="page">当前页面指针</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionHelper.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			update.Update<RefTable>(dic, wheres);
			...
			if (isAddUseLog){
				...
				{
					//添加用户操作记录
					CommonBll.WriteLog("{0}修改了RefTable表记录。");
					//UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}修改了RefTable表记录。");				
				}
				else
				{
					//添加用户操作记录
		...
		#region 删除RefTable表指定Taxa_Id的字段值记录
		/// <summary>
		...
		#region 更新RefTable表指定Taxa_Id的字段值
        /// <summary>更新RefTable表记录指定字段值，如果使用了缓存，保存成功后会清空本表的所有缓存记录，然后重新加载进缓存</summary>
        /// <param name="page">当前页面指针</param>
	    /// <param name="Taxa_Id">字段Taxa_Id的值</param>
	    /// <param name="dic">需要更新的字段与值</param>
	    /// <param name="content">更新说明</param>
        /// <param name="isCache">是否同步更新缓存</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
	    public void UpdateValue_For_Taxa_Id(Page page, long Taxa_Id, Dictionary<string, object> dic, string content = "", bool isCache = true, bool isAddUseLog = true)
        {
			content = content != "" ? content : "{0}修改了RefTable表外键Taxa_Id值为" + Taxa_Id + "的所有记录。";
			
            //条件
            var wheres = new List<ConditionHelper.SqlqueryCondition>
            {
                new ConditionHelper.SqlqueryCondition(ConstraintType.And, RefTableTable.Taxa_Id, Comparison.Equals, Taxa_Id)
            };
        ...
        /// <param name="isCache">是否同步更新缓存</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
	    public void UpdateValue_For_Taxa_Id(Page page, long Taxa_Id, string columnName, object columnValue, string content = "", bool isCache = true, bool isAddUseLog = true)
        {
			content = content != "" ? content : "{0}修改了RefTable表外键Taxa_Id值为" + Taxa_Id + "的所有记录，将" + columnName + "字段值修改为" + columnValue;
            //设置更新字段
            var dic = new Dictionary<string, object>();
            dic.Add(columnName, columnValue);

			//执行更新
            UpdateValue_For_Taxa_Id(page, Taxa_Id, dic, content, isCache, isAddUseLog);
        }
        ...
        /// <param name="isCache">是否同步更新缓存</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
	    public void UpdateValue_For_Taxa_Id(Page page, long Taxa_Id, string columnName1, object columnValue1, string columnName2, object columnValue2, string content = "", bool isCache = true, bool isAddUseLog = true)
        {
			content = content != "" ? content : "{0}修改了RefTable表外键Taxa_Id值为" + Taxa_Id + "的所有记录，将" + columnName1 + "字段值修改为" + columnValue1 + "，" + columnName2 + "字段值修改为" + columnValue2;
            //设置更新字段
            var dic = new Dictionary<string, object>();
		...
		#region 删除RefTable表指定Article_Id的字段值记录
		/// <summary>
		...
		#region 更新RefTable表指定Article_Id的字段值
        /// <summary>更新RefTable表记录指定字段值，如果使用了缓存，保存成功后会清空本表的所有缓存记录，然后重新加载进缓存</summary>
        /// <param name="page">当前页面指针</param>
	    /// <param name="Article_Id">字段Article_Id的值</param>
	    /// <param name="dic">需要更新的字段与值</param>
	    /// <param name="content">更新说明</param>
        /// <param name="isCache">是否同步更新缓存</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
	    public void UpdateValue_For_Article_Id(Page page, long Article_Id, Dictionary<string, object> dic, string content = "", bool isCache = true, bool isAddUseLog = true)
        {
			content = content != "" ? content : "{0}修改了RefTable表外键Article_Id值为" + Article_Id + "的所有记录。";
			
            //条件
            var wheres = new List<ConditionHelper.SqlqueryCondition>
            {
                new ConditionHelper.SqlqueryCondition(ConstraintType.And, RefTableTable.Article_Id, Comparison.Equals, Article_Id)
            };
        ...
        /// <param name="isCache">是否同步更新缓存</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
	    public void UpdateValue_For_Article_Id(Page page, long Article_Id, string columnName, object columnValue, string content = "", bool isCache = true, bool isAddUseLog = true)
        {
			content = content != "" ? content : "{0}修改了RefTable表外键Article_Id值为" + Article_Id + "的所有记录，将" + columnName + "字段值修改为" + columnValue;
            //设置更新字段
            var dic = new Dictionary<string, object>();
            dic.Add(columnName, columnValue);

			//执行更新
            UpdateValue_For_Article_Id(page, Article_Id, dic, content, isCache, isAddUseLog);
        }
        ...
        /// <param name="isCache">是否同步更新缓存</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
	    public void UpdateValue_For_Article_Id(Page page, long Article_Id, string columnName1, object columnValue1, string columnName2, object columnValue2, string content = "", bool isCache = true, bool isAddUseLog = true)
        {
			content = content != "" ? content : "{0}修改了RefTable表外键Article_Id值为" + Article_Id + "的所有记录，将" + columnName1 + "字段值修改为" + columnValue1 + "，" + columnName2 + "字段值修改为" + columnValue2;
            //设置更新字段
            var dic = new Dictionary<string, object>();
            dic.Add(columnName1, columnValue1);
            dic.Add(columnName2, columnValue2);
