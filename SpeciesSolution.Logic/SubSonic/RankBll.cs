...
namespace SpeciesSolution.Logic.SpeciesQuery {
	...
	/// Rank表逻辑类
	/// </summary>
	public partial class RankBll : LogicBase {
		...
		private const string const_CacheKey = "Cache_Rank";
        private const string const_CacheKey_Date = "Cache_Rank_Date";

		#region 单例模式
		//定义单例实体
		private static RankBll _RankBll = null;

		/// <summary>
		/// 获取本逻辑类单例
		/// </summary>
		/// <returns></returns>
		public static RankBll GetInstence() {
			if (_RankBll == null) {
				_RankBll = new RankBll();
			}
			return _RankBll;
		}
		#endregion
        ...
        public DataAccess.Model.Rank GetModel(long id, bool isCache = true)
        {
            //判断是否使用缓存
		    if (CommonBll.IsUseCache() && isCache)
		    {
		    ...
		    {
                //从数据库中直接读取
                var model = Rank.SingleOrDefault(x => x.Id == id);
                if (model == null)
                {
                    return null;
                }
        ...
        /// <param name="id">主键Id</param>
		/// <returns>DataAccess.Model.Rank</returns>
        public DataAccess.Model.Rank GetModelForCache(long id)
        {
			try
			{
				//从缓存中读取指定Id记录
				...
				if (model == null){
					//从数据库中读取
					var tem = Rank.SingleOrDefault(x => x.Id == id);
					if (tem == null)
					{
						return null;
					}
        ...
        /// <returns>DataAccess.Model.Rank</returns>
        public DataAccess.Model.Rank GetModelForCache(Expression<Func<DataAccess.Model.Rank, bool>> expression)
        {
			//从缓存中读取记录列表
			var list = GetList();
            //如果条件为空，则查询全表所有记录
        ...
        public void SetModelForCache(DataAccess.Model.Rank model)
        {
		    ...
		    if (list == null)
		    {
                list = new List<DataAccess.Model.Rank>();
		    }
            //添加记录
            list.Add(model);
        }
        ...
        /// </summary>
        /// <param name="model">记录实体</param>
        public void SetModelForCache(Rank model)
        {
		...
		/// <summary>
        /// 按条件删除IIS缓存中Rank表的指定记录
        /// </summary>
        /// <param name="expression">条件，值为null时删除全有记录</param>
		public void DelCache(Expression<Func<DataAccess.Model.Rank, bool>> expression)
        {
            //从缓存中获取List
		    var list = GetList();
            //如果缓存为null，则不做任何处理
		...
		#region 实体转换
		/// <summary>
		/// 将Rank记录实体（SubSonic实体）转换为普通的实体（DataAccess.Model.Rank）
		/// </summary>
        /// <param name="model">SubSonic插件生成的实体</param>
		/// <returns>DataAccess.Model.Rank</returns>
		public DataAccess.Model.Rank Transform(Rank model)
        {			
			if (model == null) 
				return null;

            return new DataAccess.Model.Rank
            {

                Id = model.Id,

                RankLevel = model.RankLevel,

                RankCName = model.RankCName,

                RankEName = model.RankEName,

            };
        }

		/// <summary>
		/// 将Rank记录实体集（SubSonic实体）转换为普通的实体集（DataAccess.Model.Rank）
		/// </summary>
        /// <param name="sourceList">SubSonic插件生成的实体集</param>
        public IList<DataAccess.Model.Rank> Transform(IList<Rank> sourceList)
        {
			//创建List容器
            var list = new List<DataAccess.Model.Rank>();
		...
		/// <summary>
		/// 将Rank记录实体由普通的实体（DataAccess.Model.Rank）转换为SubSonic插件生成的实体
		/// </summary>
        /// <param name="model">普通的实体（DataAccess.Model.Rank）</param>
		/// <returns>Rank</returns>
		public Rank Transform(DataAccess.Model.Rank model)
        {
			if (model == null) 
				return null;
		...
		/// <summary>
		/// 将Rank记录实体由普通实体集（DataAccess.Model.Rank）转换为SubSonic插件生成的实体集
		/// </summary>
        /// <param name="sourceList">普通实体集（DataAccess.Model.Rank）</param>
        public IList<Rank> Transform(IList<DataAccess.Model.Rank> sourceList)
        {
			//创建List容器
		...
		public void SetModelValue(DataAccess.Model.Rank model, Dictionary<string, object> dic)
		{
		...
		public void SetModelValue(DataAccess.Model.Rank model, string colName, object value)
		{
            ...
            switch (colName)
            {
				...
				case "RankLevel" :
					model.RankLevel = (string)value;
                    break;

				case "RankCName" :
					model.RankCName = (string)value;
                    break;

				case "RankEName" :
					model.RankEName = (string)value;
                    break;
        ...
        public int GetRecordCount()
        {
			...
			else
			{
				...
				return select.GetRecordCount<Rank>();
			}
        }
		...
		public int GetRecordCount(List<ConditionHelper.SqlqueryCondition> wheres) {
			var select = new SelectHelper();
			return select.GetRecordCount<Rank>(wheres);

		}

		/// <summary>
		/// 获取Rank表指定条件的记录总数——从数据库中查询
		/// </summary>
        /// <param name="expression">条件</param>
		/// <returns>int</returns>
		public int GetRecordCount(Expression<Func<Rank, bool>> expression) {
            return new Select().From<Rank>().Where(expression).GetRecordCount();
		}
        ...
        public IList<DataAccess.Model.Rank> Find(Expression<Func<DataAccess.Model.Rank, bool>> expression)
        {
			//从缓存中获取记录集
			var list = GetList();
            //判断获取记录集是否为null
        ...
        /// <param name="expression">条件语句</param>
        /// <returns></returns>
        public bool Exist(Expression<Func<DataAccess.Model.Rank, bool>> expression)
        {
            var list = GetList();
            if (list == null) 
                return false;
		...
		public DataTable GetDataTable(bool norepeat = false, int top = 0, List<string> columns = null, int pageIndex = 0, int pageSize = 0, List<ConditionHelper.SqlqueryCondition> wheres = null, List<string> sorts = null) {
			try
            {
                //分页查询
                var select = new SelectHelper();
                return select.SelectDataTable<Rank>(norepeat, top, columns, pageIndex, pageSize, wheres, sorts);
            }
            catch (Exception e)
            {
                //记录日志
                CommonBll.WriteLog("获取Rank表记录时出现异常", e);
		...
		/// <param name="model">Rank表实体</param>
        /// <param name="content">更新说明</param>
        /// <param name="isCache">是否更新缓存</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
        public void Save(Page page, Rank model, string content = null, bool isCache = true, bool isAddUseLog = true)
        {
			try {
				//保存
				model.Save();
				...
				if (isAddUseLog)
				{
					...
					{
						content = "{0}" + (model.Id == 0 ? "添加" : "编辑") + "Rank记录成功，ID为【" + model.Id + "】";
					}
				...
				var result = "执行RankBll.Save()函数出错！";
		...
		{
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} = {2}", RankTable.TableName,  RankTable.Id, id);
		    ...
		    {
				//添加用户操作记录
				CommonBll.WriteLog("{0}删除了Rank表id为【" + id + "】的记录！");
				//UseLogBll.GetInstence().Save(page, "{0}删除了Rank表id为【" + id + "】的记录！");
			}
		}
		...
		public  void Delete(Page page, int[] id, bool isAddUseLog = true) 
		{
			...
			//设置Sql语句
			var sql = string.Format("delete from {0} where {1} in ({2})", RankTable.TableName,  RankTable.Id, str);
		    ...
		    {
				//添加用户操作记录
				CommonBll.WriteLog("{0}删除了Rank表id为【" + str + "】的记录！");
				//UseLogBll.GetInstence().Save(page, "{0}删除了Rank表id为【" + str + "】的记录！");
			}
		}

		/// <summary>
        /// 删除Rank表记录——如果使用了缓存，删除成功后会清空本表的所有缓存记录，然后重新加载进缓存
        /// </summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="expression">条件语句</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
        public void Delete(Page page, Expression<Func<Rank, bool>> expression, bool isAddUseLog = true)
        {
			//执行删除
			Rank.Delete(expression);
		    ...
		    {
				//添加用户操作记录
				CommonBll.WriteLog("{0}删除了Rank表记录！");
				//UseLogBll.GetInstence().Save(page, "{0}删除了Rank表记录！");
			}
        }
        ...
        {
            //设置Sql语句
            var sql = string.Format("delete from {0}", RankTable.TableName);
            ...
            if (isAddUseLog)
            {
                //添加用户操作记录
				CommonBll.WriteLog("{0}删除了Rank表记录！");
                //UseLogBll.GetInstence().Save(page, "{0}删除了Rank表所有记录！");
            }
        }
		#endregion
	    ...
	    {
		     //更新排序
			if (CommonBll.UpdateSort(page, grid1, tbxSort, "Rank", sortName, "Id"))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				CommonBll.WriteLog("{0}更新了Rank表排序！");
				//UseLogBll.GetInstence().Save(page, "{0}更新了Rank表排序！");
	    ...
	    {
		    //更新排序
			if (CommonBll.AutoSort("Id", "Rank", strWhere, isExistsMoreLv, pid, fieldName, fieldParentId))
		    {
				//判断是否启用缓存
                if (CommonBll.IsUseCache())
                {
			    ...
			    //添加用户操作记录
				CommonBll.WriteLog("{0}对Rank表进行了自动排序操作！");
				//UseLogBll.GetInstence().Save(page, "{0}对Rank表进行了自动排序操作！");
        ...
        /// <param name="expression">条件</param>
        /// <returns></returns>
        public object GetFieldValue(string colName, Expression<Func<DataAccess.Model.Rank, bool>> expression)
	    {
        ...
        {
            var select = new SelectHelper();
            return select.GetColumnsValue<Rank>(colName, wheres);
        }
        ...
        /// <returns></returns>
		private object GetFieldValue(DataAccess.Model.Rank model, string colName)
		{
			...
			switch (colName)
			{
				...
				case "RankLevel" :
					return model.RankLevel;

				case "RankCName" :
					return model.RankCName;

				case "RankEName" :
					return model.RankEName;
		...
		#region 更新Rank表指定字段值
		/// <summary>更新Rank表记录指定字段值，如果使用了缓存，保存成功后会清空本表的所有缓存记录，然后重新加载进缓存</summary>
		/// <param name="page">当前页面指针</param>
		...
		public void UpdateValue(Page page, Dictionary<string, object> dic, List<ConditionHelper.SqlqueryCondition> wheres = null, string content = "", bool isCache = true, bool isAddUseLog = true) {
			...
			if (isAddUseLog){
				...
				{
					//添加用户操作记录
					CommonBll.WriteLog("{0}修改了Rank表记录。");
					//UseLogBll.GetInstence().Save(page, content != "" ? content : "{0}修改了Rank表记录。");				
				}
				else
				{
					//添加用户操作记录
		...
		#region 更新Rank表指定主键Id的字段值
		/// <summary>更新Rank表记录指定字段值</summary>
        /// <param name="page">当前页面指针</param>
        ...
        {
			content = content != "" ? content : "{0}修改了Rank表主键Id值为" + id + "的记录。";
            ...
            if (id > 0)
            {
                ...
                wheres.Add(new ConditionHelper.SqlqueryCondition(ConstraintType.And, RankTable.Id, Comparison.Equals, id));
            };
        ...
        /// <summary>更新Rank表记录指定字段值（更新一个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了Rank表主键Id值为" + id + "的记录，将" + columnName + "字段值修改为" + columnValue;
            //设置更新字段
		 ...
		 /// <summary>更新Rank表记录指定字段值（更新两个字段值）</summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="id">主键Id，当小于等于0时，则更新所有记录</param>
        ...
        {
            content = content != "" ? content : "{0}修改了Rank表主键Id值为" + id + "的记录，将" + columnName1 + "字段值修改为" + columnValue1 + "，" + columnName2 + "字段值修改为" + columnValue2;
            //设置更新字段
            var dic = new Dictionary<string, object>();
        ...
        /// <param name="isCache">是否从缓存中读取</param>
        /// <returns></returns>
        public string GetRankCName(Page page, int pkValue, bool isCache = true)
        {
            //判断是否启用缓存
            if (isCache && CommonBll.IsUseCache())
            {
                ...
                return model == null ? "" : model.RankCName;
            }
            else
            {
                //从数据库中查询
                var model = Rank.SingleOrDefault(x => x.Id == pkValue);
                return model == null ? "" : model.RankCName;
            }
        }
        #endregion
        ...
        /// <param name="isCache">是否从缓存中读取</param>
        /// <returns></returns>
        public string GetRankEName(Page page, int pkValue, bool isCache = true)
        {
            //判断是否启用缓存
            if (isCache && CommonBll.IsUseCache())
            {
                //从缓存中获取实体
                var model = GetModelForCache(pkValue);
                return model == null ? "" : model.RankEName;
            }
            else
            {
                //从数据库中查询
